...
def deeid(
    ...
    layout = BIDSLayout(bids_dir, validate=not skip_bids_validation, derivatives=False)
    ...
    for subject_label in subjects_to_analyze:
        ...
        for t1w in list_t1w:
            ...
            if brainextraction == "bet":
                brainmask = run_brain_extraction_bet(
                    source_t1w, bet_frac[0], brainmask_path
                )
            elif brainextraction == "nobrainer":
                brainmask = run_brain_extraction_nb(source_t1w, brainmask_path)
            ...
            if deface_t2w:
                ...
                if len(t2w) == 0:
                    logger.warn(
                        ...
                        f"However, the T2w image {t2w} doesn't exist for T1 image {t1w}"
                    )
                else:
                    t2w = t2w[0]
                    T2_file = t2w.path
                    ...
                    if brainextraction == "bet":
                        run_brain_extraction_bet(
                            source_t2w, bet_frac[0], subject_label, bids_dir
                        )
                    elif brainextraction == "nobrainer":
                        run_brain_extraction_nb(source_t2w, subject_label, bids_dir)
